...
const syncWithGlobal = (nextItems: Indicator[]) => {
  ...
  const bulletLines = nextItems
    ...
    .map(i => {
      ...
      // convert HTML to text but preserve <a> as clickable markers for DOCX
      ...
      const sanitized = convert(i.summary ?? '');
    ...
    <div className="flex flex-wrap gap-2">
      ...
      <button type="button" onClick={() => {
          ...
          if (sel && sel.rangeCount > 0 && within) {
            ...
            if (range.collapsed) {
              try { if (expandSelectionToWord(range)) { sel.removeAllRanges(); sel.addRange(range); } } catch {}
            }
