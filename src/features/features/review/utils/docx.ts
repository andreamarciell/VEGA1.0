  ...
  return {
    ...
    reputationalIndicatorsRich: (() => {
      ...
      return lines.map((line: string, idx: number) => {
        ...
        return {
          prefix: prefixText,
          authorLabel: author || (s.source || '').trim() || 'link',
          suffix: suffix,
          link,
        };
      });
...
return tagValue as ArrayBuffer;
  ...
  const _raw = doc.getZip().generate({ type: 'blob', mimeType: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' });
  const _post = await postprocessDocxHyperlinks(_raw);
  return _post;
}
