...
// src/features/review/components/SectionForms/ReputationalIndicatorsForm.tsx
// NOTE: This is a focused replacement that ensures the editable editor content includes the
// "Secondo l'articolo di ..." line and that links created in TipTap are preserved in HTML.
...
type IndicatorItem = {
  ...
  url?: string;
  date?: string;
  match?: string; // e.g., "corrispondenza definitiva via nome + età + area + foto"
  summaryHtml?: string; // legacy field
  contentHtml?: string; // full rich html (what we persist)
};
...
export default function ReputationalIndicatorsForm() {
  ...
  const savedItems: IndicatorItem[] = (state?.adverseData as any)?.reputationalIndicatorsItems ?? [];
  const [items, setItems] = useState<IndicatorItem[]>(savedItems);

  // hydrate from persisted (also when it arrives async from localStorage)
  useEffect(() => {
    const persisted: IndicatorItem[] = (state?.adverseData as any)?.reputationalIndicatorsItems ?? [];
  ...
  function setField<K extends keyof IndicatorItem>(k: K, v: IndicatorItem[K]) {
    ...
    // if content is empty OR still starts with previous auto prefix, rebuild prefix
  ...
  function saveToGlobal() {
    ...
    markSectionComplete("reputationalIndicators", hasAny);
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {current && (
        <div className="space-y-2">
          <div className="grid grid-cols-2 gap-2">
            ...
            <input className="border rounded p-2" placeholder="Corrispondenza (es. definitiva via nome + età + area + foto)" value={current.match ?? ""} onChange={(e) => setField("match", e.target.value)} />
          </div>
      ...
      <div className="text-sm text-gray-500">Gli indicatori sono salvati e persistono quando navighi tra le pagine.</div>
    </div>
  );
}
