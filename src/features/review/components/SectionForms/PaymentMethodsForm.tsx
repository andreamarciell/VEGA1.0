...
const PaymentMethodsForm: React.FC = () => {
  ...
  const data = safeArray(state?.fullData?.paymentMethods);

  useEffect(() => {
    const complete = data.some((d) => typeof d?.type === 'string' && d.type.trim() !== '');
    markSectionComplete('payment-methods', complete);
  }, [data]);

  const handleChange = (index: number, field: string, value: string) => {
    const updated = [...data];
    updated[index] = { ...updated[index], [field]: value };
    updateFullData({ paymentMethods: updated });
  };

  const addRow = () => updateFullData({ paymentMethods: [...data, { nameNumber: '', type: '', additionalInfo: '' }] });

  const removeRow = (index: number) =>
    updateFullData({ paymentMethods: data.filter((_, i) => i !== index) });

  return (
    <div className="bg-white rounded-xl shadow-md p-6">
      <h2 className="text-2xl font-bold mb-6">Metodi di Pagamento</h2>
      <div className="space-y-4">
        ...
        <button onClick={addRow} className="flex items-center gap-2 border-2 border-dashed px-4 py-3 rounded-lg">
          <Plus className="w-4 h-4" /> Aggiungi metodo di pagamento
        </button>
      </div>
    </div>
  );
