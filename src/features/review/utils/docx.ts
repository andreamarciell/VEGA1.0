...
export function htmlToPlainKeepUrls(h:string){
  ...
  // FIX: JS doesn't support inline (?i). Use the 'i' flag on the regex instead.
...
async function ensureArrayBuffer(input:any):Promise<ArrayBuffer>{
  ...
  if(input&&typeof input==="object"&&!("byteLength"in input)&&!("arrayBuffer"in input)){
    const candidates=[input.templateBinary,input.templateArrayBuffer,input.template,input.templateUrl,input.url,input.href,input.src,input.path];
    for(const c of candidates){ if(c) return ensureArrayBuffer(c); }
    for(const k of Object.keys(input)){
      const v=(input as any)[k];
      if(typeof v==="string" && /\.docx(\?.*)?$/i.test(v)) return ensureArrayBuffer(v);
      if(v&&typeof v==="object" && (v.name?.endsWith?.(".docx") || v.type==="application/vnd.openxmlformats-officedocument.wordprocessingml.document"))
  ...
  if(typeof input==="string"){
    ...
    if(typeof fetch==="function" && (/^https?:/.test(input) || input.startsWith("/") || input.startsWith("./") || input.startsWith("../"))){
      const res=await fetch(input); return await res.arrayBuffer();
    }
  }
...
export async function exportToDocx(a:any,b?:any,c?:any):Promise<Blob>{
  if(arguments.length===1 && a && typeof a==="object" && (a.template||a.templateBinary||a.templateArrayBuffer||a.templateUrl||a.url||a.href||a.src||a.path)){
    const {template,templateBinary,templateArrayBuffer,templateUrl,url,href,src,path,data,imageOpts}=a as any;
    const tpl=templateBinary??templateArrayBuffer??template??templateUrl??url??href??src??path;
