...
export const DEVELOPMENT_CSP_OVERRIDES = {
  'Content-Security-Policy': [
    ...
    "font-src 'self' data:",
    // Allow dev server and Clerk connections (including telemetry), plus Google Cloud Storage
    "connect-src 'self' https://*.clerk.accounts.dev https://enhanced-parakeet-13.clerk.accounts.dev https://clerk-telemetry.com https://storage.googleapis.com localhost:* ws: wss:",
    // Allow Clerk iframes
    "frame-src 'self' https://*.clerk.accounts.dev",
    // Allow Web Workers (needed for Clerk)
    "worker-src 'self' blob:",
