...
export interface ProcessedCard {
  bin: string; pan: string; name: string; type: string; prod: string;
  ctry: string; bank: string; app: number; dec: number; nDec: number; reasons: string[];
}
...
export interface MovementsData {
  totAll: number; months: string[]; all: Record<string, number>;
  perMonth: Record<string, Record<string, number>>; frazionate: Frazionata[];
}
...
const parseNum = (v: any) => { if (typeof v === 'number') return isFinite(v) ? v : 0; if (v == null) return 0; let s = String(v).trim().replace(/\s+/g, ''); const ld = s.lastIndexOf('.'); const lc = s.lastIndexOf(','); if (lc > -1 && ld > -1) { s = (lc > ld) ? s.replace(/\./g, '').replace(/,/g, '.') : s.replace(/,/g, ''); } else if (lc > -1) { s = s.replace(/\./g, '').replace(/,/g, '.'); } const n = parseFloat(s.replace(/[^0-9.-]/g, '')); return isNaN(n) ? 0 : n; };
const excelToDate = (d: any): Date | null => { if (d instanceof Date) return d; if (typeof d === 'number') { const base = new Date(1899, 11, 30, 0, 0, 0); base.setDate(base.getDate() + d); return base; } if (typeof d === 'string') { const s = d.trim(); const m = s.match(/^([0-3]?\d)[\\/\\-]([0-1]?\d)[\\/\\-](\d{2,4})(?:\D+([0-2]?\d):([0-5]?\d)(?::([0-5]?\d))?)?/); if (m) { let [, day, mon, yr, hh, mm, ss] = m.map(Number); yr = yr < 100 ? yr + 2000 : yr; return new Date(yr, mon - 1, day, hh || 0, mm || 0, ss || 0); } const tryDate = new Date(s); if (!isNaN(tryDate.getTime())) return tryDate; } return null; };
const monthKey = (dt: Date) => dt.getFullYear() + '-' + String(dt.getMonth() + 1).padStart(2, '0');
const readExcel = (file: File): Promise<any[][]> => new Promise((res, rej) => { const fr = new FileReader(); fr.onload = e => { try { const wb = XLSX.read(new Uint8Array(e.target!.result as ArrayBuffer), { type: 'array' }); const rows = XLSX.utils.sheet_to_json(wb.Sheets[wb.SheetNames[0]], { header: 1 }); res(rows); } catch (err) { rej(err); } }; fr.onerror = rej; fr.readAsArrayBuffer(file); });

function calcWithdrawFrazionate(rows: any[][], cDate: number, cDesc: number, cAmt: number): Frazionata[] {
  const THRESHOLD = 5000;
  const isVoucherPVR = (desc: any) => { const d = String(desc ?? '').toLowerCase(); return d.includes('voucher') && d.includes('pvr'); };
  const txs: Array<{ data: Date; importo: number; importo_raw: any; causale: string }> = [];
  rows.forEach(r => { if (!Array.isArray(r)) return; const desc = String(r[cDesc] ?? '').trim(); if (!isVoucherPVR(desc)) return; const amt = parseNum(r[cAmt]); if (!amt) return; const dt = excelToDate(r[cDate]); if (!dt || isNaN(dt.getTime())) return; txs.push({ data: dt, importo: Math.abs(amt), importo_raw: r[cAmt], causale: desc }); });
  txs.sort((a, b) => a.data.getTime() - b.data.getTime());
  const startOfDay = (d: Date) => { const t = new Date(d); t.setHours(0, 0, 0, 0); return t; };
  const fmtDateLocal = (d: Date) => { const y = d.getFullYear(); const m = String(d.getMonth() + 1).padStart(2, '0'); const da = String(d.getDate()).padStart(2, '0'); return `${y}-${m}-${da}`; };
  const res: Frazionata[] = []; let i = 0;
  while (i < txs.length) { const windowStart = startOfDay(txs[i].data); let j = i, run = 0; while (j < txs.length) { const t = txs[j]; const diffDays = (startOfDay(t.data).getTime() - windowStart.getTime()) / (1000 * 60 * 60 * 24); if (diffDays > 6) break; run += t.importo; if (run >= THRESHOLD) { res.push({ start: fmtDateLocal(windowStart), end: fmtDateLocal(startOfDay(t.data)), total: run, transactions: txs.slice(i, j + 1).map(tx => ({ date: tx.data.toISOString(), amount: tx.importo, raw: tx.importo_raw, causale: tx.causale })) }); i = j + 1; break; } j++; } if (run < THRESHOLD) i++; }
  return res;
}
...
async function parseMovements(file: File, mode: 'deposit' | 'withdraw'): Promise<MovementsData> {
  ...
  let totAll = 0;
  data.forEach(r => { if (!Array.isArray(r)) return; const desc = String(r[cDesc] ?? '').trim(); if (!RE.test(desc)) return; const method = mode === 'deposit' && desc.toLowerCase().startsWith('ricarica') ? 'Cash' : desc.replace(RE, '').trim() || 'Sconosciuto'; const amt = parseNum(r[cAmt]); if (!amt) return; all[method] = (all[method] || 0) + amt; totAll += amt; const dt = excelToDate(r[cDate]); if (!dt || isNaN(dt.getTime())) return; const k = monthKey(dt); perMonth[method] ??= {}; perMonth[method][k] = (perMonth[method][k] || 0) + amt; });
...
function processCardData(rows: any[][]): CardDataProcessed {
  ...
  const monthsSet = new Set<string>();
  data.forEach(r => { const txType = String(r[ix.ttype] || '').toLowerCase(); if (!txType.includes('sale')) return; const dt = excelToDate(r[ix.date]); if (dt && !isNaN(dt.getTime())) { monthsSet.add(monthKey(dt)); } const pan = r[ix.pan] || 'UNKNOWN'; cards[pan] ??= { bin: ix.bin !== -1 ? (r[ix.bin] || String(pan).slice(0, 6)) : '', pan, name: ix.name !== -1 ? (r[ix.name] || '') : '', type: ix.type !== -1 ? (r[ix.type] || '') : '', prod: ix.prod !== -1 ? (r[ix.prod] || '') : '', ctry: ix.ctry !== -1 ? (r[ix.ctry] || '') : '', bank: ix.bank !== -1 ? (r[ix.bank] || '') : '', app: 0, dec: 0, nDec: 0, reasonsSet: new Set(), reasons: [] }; const amt = parseNum(r[ix.amt]); const resVal = ix.res !== -1 ? String(r[ix.res] || '') : 'approved'; if (/^approved$/i.test(resVal)) { cards[pan].app += amt; summary.app += amt; } else { cards[pan].dec += amt; summary.dec += amt; cards[pan].nDec += 1; if (ix.reason !== -1 && r[ix.reason]) cards[pan].reasonsSet.add(r[ix.reason]); } });
...
// Tipi di dati originali per l'analisi principale ("Giasai")
interface Transaction { data: Date; dataStr: string; causale: string; importo: number; importo_raw: any; TSN?: string; "TS extension"?: string; }
interface AmlResults { riskScore: number; riskLevel: string; motivations: string[]; frazionate: Frazionata[]; patterns: string[]; alerts: string[]; sessions: Array<{ timestamp: string; }>; }
...
const AmlDashboard = () => {
  ...
  // STATI PER ANALISI PRINCIPALE ("GIASAI")
  ...
  // =========================================================================
  // ====== LOGICA ANALISI PRINCIPALE ("GIASAI") - MANTENUTA INTATTA ======
  // =========================================================================
  ...
  const handleFile = (event: React.ChangeEvent<HTMLInputElement>) => { const file = event.target.files?.[0]; if (!file) return; const reader = new FileReader(); reader.onload = function (e) { try { const data = new Uint8Array(e.target!.result as ArrayBuffer); const workbook = XLSX.read(data, { type: 'array' }); const sheetName = workbook.SheetNames[0]; const sheet = workbook.Sheets[sheetName]; const jsonData = XLSX.utils.sheet_to_json(sheet, { header: 1 }); let headerIdx = 0; for (let i = 0; i < jsonData.length; i++) { const r = jsonData[i] as any[]; if (!r || r.length < 9) continue; const c7 = String(r[7] || '').toLowerCase(); const c8 = String(r[8] || '').toLowerCase(); if ((c7.includes('caus') || c7.includes('reason')) && (c8.includes('importo') || c8.includes('amount'))) { headerIdx = i; break; } } const headerRow = (jsonData[headerIdx] as any[] || []).map(h => typeof h === 'string' ? h.trim() : h); const tsIndex = headerRow.findIndex(h => { if (!h) return false; const norm = String(h).toLowerCase().replace(/\s+/g, ''); return norm.includes('tsn') || norm.includes('tsextension'); }); const rows = (jsonData.slice(headerIdx + 1) as any[][]).filter(row => row.length >= 9 && row[0] && row[7] && row[8]); const parsedTransactions = rows.map(row => { const dataStr = row[0]; const causale = row[7]; const importo = parseFloat(String(row[8]).replace(/\s+/g, '').replace(/\./g, '').replace(/,/g, '.')); const dataObj = parseDate(dataStr); const tsVal = tsIndex !== -1 ? row[tsIndex] : ''; const tx: Transaction = { data: dataObj, dataStr: dataStr, causale: causale, importo: importo, importo_raw: row[8] }; if (tsIndex !== -1 && tsVal != null && tsVal !== '') { tx["TSN"] = tsVal; tx["TS extension"] = tsVal; } return tx; }).filter(tx => tx.data instanceof Date && !isNaN(tx.data.getTime())); const sessionTsData = parsedTransactions.map(tx => ({ timestamp: tx.data.toISOString() })); if (parsedTransactions.length > 0) { setTransactions(parsedTransactions); setSessionTimestamps(sessionTsData); toast.success(`${parsedTransactions.length} transazioni caricate con successo`); } else { toast.error('Nessuna transazione valida trovata nel file'); } } catch (error) { console.error('Error parsing file:', error); toast.error('Errore durante la lettura del file Excel'); } }; reader.readAsArrayBuffer(file); };
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {!results ? (
          ...
          <div className="space-y-6">
            <div className="flex justify-between items-center"><h2 className="text-2xl font-semibold">Risultati Analisi</h2><Button onClick={handleReset} variant="outline">Nuova Analisi</Button></div>
