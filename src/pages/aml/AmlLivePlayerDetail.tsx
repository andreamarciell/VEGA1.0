...
import { ArrowLeft, ChevronDown, ChevronRight, Gift, Info, Loader2 } from 'lucide-react';
import { toast } from 'sonner';
// Rimossa import XLSX - non necessario per Live (dati dal DB)
// @ts-ignore
...
// Non è più necessario un ricalcolo manuale separato.
  ...
  useEffect(() => {
    const initialize = async () => {
      try {
        ...
        // Sincronizza dati dal database
        setIsLoading(true);
        const data = await syncFromDatabase(accountId);
        
        if (!data) {
          toast.error('Errore nel caricamento dati dal database');
          setIsLoading(false);
          return;
        }
        ...
        const transactionsData: Transaction[] = data.transactions.map((tx: any) => {
          const dataDate = tx.data instanceof Date ? tx.data : new Date(tx.data || tx.dataStr);
          return {
            data: dataDate,
            dataStr: tx.dataStr || tx.data || dataDate.toISOString(),
            causale: tx.causale || '',
            importo: tx.importo || 0,
            importo_raw: tx.importo_raw || tx.importo,
            ...(tx.TSN && { TSN: tx.TSN, "TS extension": tx.TSN }),
            deposit_domain: tx.deposit_domain,
            withdrawal_mode: tx.withdrawal_mode
          };
        });

        // Calcola durationMinutes per sessioni Casino Live (stessa logica di AmlDashboard)
        ...
        const transactionsWithDuration = calculateCasinoLiveDuration(transactionsData);
        setTransactions(transactionsWithDuration);
        localStorage.setItem('amlTransactions', JSON.stringify(transactionsWithDuration));

        // Separa depositi e prelievi
        const classifyTransaction = (tx: Transaction): 'deposit' | 'withdrawal' | 'other' => {
          const causale = tx.causale.toLowerCase();
          const hasDeposit = causale.includes('ricarica') || 
                            causale.includes('deposito') || 
                            causale.includes('deposit') ||
                            causale.includes('accredito');
          const hasWithdrawal = causale.includes('prelievo') || 
                               causale.includes('withdraw');
        ...
        const depositsList = transactionsWithDuration.filter(tx => 
          classifyTransaction(tx) === 'deposit'
        );
        const withdrawalsList = transactionsWithDuration.filter(tx => 
          classifyTransaction(tx) === 'withdrawal'
        );

        setDeposits(depositsList);
        setWithdrawals(withdrawalsList);

        // Crea sessionTimestamps
        const sessions = transactionsWithDuration.map(tx => ({
        ...
        await runAnalysis(transactionsWithDuration);
  ...
  // Rimossi: accessFile, isAnalyzingAccess - non necessari per Live (dati dal DB)
  const accessResults = useAmlStore(state => state.accessResults);
  const setAccessResults = useAmlStore(state => state.setAccessResults);
  // Rimossa: fileInputRef - non necessario per Live (dati dal DB)
...
useEffect(() => {
  ...
  // Rimossa: handleFile - non necessario per Live (dati dal DB)
  // I dati vengono caricati automaticamente dal database in useEffect
  ...
  // Original runAnalysis function - aggiornata per accettare transactions come parametro
  const runAnalysis = async (txs?: Transaction[]) => {
    const txsToUse = txs || transactions;
    if (txsToUse.length === 0) {
      toast.error('Nessuna transazione disponibile');
      return;
    }
    setIsAnalyzing(true);
    try {
      // Persist transactions to localStorage so other tabs can access them reliably.
      localStorage.setItem('amlTransactions', JSON.stringify(txsToUse));

      // Calcola frazionate depositi immediatamente (priorità massima)
      const frazionateDep = cercaFrazionate(txsToUse);
      
      // Calcola frazionate prelievi direttamente su transactions (filtro: "voucher" o "pvr")
      const frazionateWit = cercaFrazionatePrelievi(txsToUse);
      
      // Calcola patterns
      const patterns = cercaPatternAML(txsToUse);
      ...
      const alerts = rilevaAlertAML(txsToUse);
      ...
      <div className="space-y-6">
            <Card className="p-8">
              <div className="text-center">
                ...
                <h2 className="text-2xl font-semibold mb-2">Caricamento Dati dal Database</h2>
                <p className="text-muted-foreground">
                  Sincronizzazione in corso per account {accountId}...
                </p>
              </div>
            </Card>
          </div>) : (
                ...
                <Card className="p-6">
                  ...
                  <div className="space-y-4">
                    <p className="text-sm text-muted-foreground">
                      Dati accessi sincronizzati dal database per account {accountId}
                    </p>
                    ...
                    {accessResults.length > 0 ? (
                            ...
                            <tbody>
                              {accessResults.map((result, index) => (
                                <tr key={index} className="hover:bg-muted/50">
                                  ...
                                  <td className="border border-border p-2">{result.paese || result.country || '-'}</td>
                      ...
                      <div className="mt-6 p-4 bg-muted rounded-lg">
                        <p className="text-sm text-muted-foreground">
                          Nessun accesso trovato per questo account
                        </p>
                      </div>
                    )}
                  </div>
...
export default AmlLivePlayerDetail;