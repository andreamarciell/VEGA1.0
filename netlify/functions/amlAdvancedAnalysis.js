...
exports.handler = async function (event) {
  ...
  try {
    ...
    if (OPENROUTER_API_KEY) {
      const sys = `Sei un analista AML per iGaming. Rispondi SOLO in JSON valido con almeno: { "summary": "...", "risk_score": 0 }.
- Non includere markdown o code fence.
- Se un'informazione non è disponibile, scrivi il meglio possibile senza inventare PII.`;
      const user = [
...
function buildFallbackSummary(txs) {
  ...
  return `l’utente ha depositato €${euro(totals.in)} ed effettuato prelievi pari a €${euro(totals.out)}. ` +
    ...
    `nel mese in esame non sono state riscontrate anomalie evidenti sui prodotti analizzati. ` +
